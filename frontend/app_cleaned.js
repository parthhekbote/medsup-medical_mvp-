...
function calculateRiskSignal(daysLeft) {
    if (daysLeft < 7) return { level: 'CRITICAL', icon: '🔴', class: 'risk-critical' };
    if (daysLeft < 14) return { level: 'WARNING', icon: '🟠', class: 'risk-warning' };
    return { level: 'STABLE', icon: '🟢', class: 'risk-stable' };
}
    ...
    if (priorityItems.length > 0) {
        priorityHTML = `
            <div style="border-top: 1px solid var(--border); padding-top: 0.75rem; margin-top: 0.75rem;">
                <div style="font-size: 0.75rem; color: var(--text-muted); margin-bottom: 0.5rem; text-transform: uppercase;">⚠️ Actions Required (Next 24-72 hrs)</div>
                ${priorityItems.map(item => `
                    <div class="priority-item ${item.risk.class}" onclick="highlightTableRow(${item.id || item.supply_id})">
                        <div class="priority-info">
                            <span class="priority-name">${item.risk.icon} ${item.name}</span>
                            <span class="priority-days">${item.daysLeft.toFixed(1)}d left</span>
                        </div>
    ...
    strip.innerHTML = `
        <div style="display: flex; gap: 1rem;">
            ...
            <div class="cmd-metric" onclick="filterInventory('CRITICAL')">
                <span class="cmd-label">⚠️ Critical (< 7d)</span>
...
function generateRecommendations(data, filterType = null) {
    ...
    if (actions.length === 0) {
        container.innerHTML = '<div class="text-muted" style="padding:1rem">✅ No pending actions.</div>';
        return;
    }
...
async function approveOrder(id, qty, name) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            logAuditAction('ORDER_APPROVED', `${user.name} (${user.role}) approved order for ${qty}x ${name}`);
...
function rejectOrder(name) {
    ...
    const user = getCurrentUser();
    logAuditAction('ORDER_REJECTED', `${user.name} (${user.role}) rejected restock for ${name}`);
}
...
async function addTransaction() {
    const customer_id = document.getElementById('txnCustomer').value;
    const type = document.getElementById('txnType').value;
    const amount = parseFloat(document.getElementById('txnAmount').value);
    const cost = parseFloat(document.getElementById('txnCost').value) || 0;

    if (!customer_id || !amount) return;

    await fetch(`${API_URL}/accounting/transactions`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ customer_id, type, amount, cost_amount: cost })
    });
